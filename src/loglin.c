...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, 
  const mxArray *prhs[])
{
    const mxArray *mxconf, *mxtable, *mxstart, *mxsnmar, *mxeps, *mxiter;
    mxArray *mxfit, *mxdev;
    double *prnmar, *priter, *preps, *prconf, *dev;
    mwSize nvar, ntab, ncon, i, nconf;
    const mwSize *dtab, *confsz;
    int nmar, maxit, nlast, ifault, *conf, *locmar;
    double eps, *fit, *u, *table, maxdev, *marg;
    if ((nlhs < 1) | (nlhs > 3))
	mexErrMsgIdAndTxt( "MATLAB:loglin:nlhs",
			   "At least one and at most 3 output arguments expected.");
    if (nrhs != 6)
	mexErrMsgIdAndTxt( "MATLAB:loglin:nrhs",
    ...
    mxeps   = prhs[4];
    mxiter  = prhs[5];
    nvar    = mxGetNumberOfDimensions(mxtable);
    dtab    = mxGetDimensions(mxtable);
    ntab    = mxGetNumberOfElements(mxtable);
    confsz  = mxGetDimensions(mxconf);
    ncon    = confsz[1]; /* error wenn 1-D? */
    nconf   = mxGetNumberOfElements(mxconf);
    prnmar  = (double *)mxGetData(mxsnmar);
    nmar    = (int)prnmar[0];
    priter  = (double *)mxGetData(mxiter);
    maxit   = (int)priter[0];
    preps   = (double *)mxGetData(mxeps);
    maxdev  = (double)preps[0];
    if ((int)mxGetNumberOfElements(mxstart) == 1) {
        mxfit = mxCreateDoubleMatrix(1, ntab, mxREAL);
        fit   = (double*)mxGetData(mxfit);
        for (i=0;i<ntab;i++)
            fit[i] = 1.0;
    }
    else {
        mxfit = mxDuplicateArray(mxstart);
        fit   = (double *)mxGetData(mxfit);
    }
    locmar = (int *)mxCalloc(ncon, sizeof(int));
    marg   = (double *)mxCalloc(nmar, sizeof(double));
    u      = (double *)mxCalloc(ntab, sizeof(double));
    mxdev  = mxCreateDoubleMatrix(1, maxit, mxREAL);
    dev    = (double *)mxGetData(mxdev);
    conf   = (int *)mxCalloc(nconf, sizeof(int));
    prconf = (double *)mxGetData(mxconf);
    for (i=0;i<nconf;i++)
        conf[i] = (int)prconf[i];
    table  = (double *)mxGetData(mxtable);
    loglin(nvar, dtab, ncon, conf, ntab,
            table, fit, locmar, nmar, marg,
            ntab, u, maxdev, maxit, dev, &nlast, &ifault);
    switch(ifault) {
    case 1:
    case 2:
	mexErrMsgIdAndTxt("MATLAB:loglin:unknown","this should not happen"); break;
    case 3:
	mexWarnMsgIdAndTxt("MATLAB:loglin:noconv","algorithm did not converge"); break;
    case 4:
	mexErrMsgIdAndTxt("MATLAB:loglin:invalid","incorrect specification of 'table' or 'start'"); break;
    default: 
	break;
    }
    plhs[0] = mxfit;
    if (nlhs > 1)
        ...
        mxDestroyArray(mxdev);
    if (nlhs > 2)
        plhs[2] = mxCreateDoubleScalar((double)nlast);
}
